...
if(!empty($_SESSION) && $_SESSION['loginType'] == "fb" ){
	header("Location: http://www.ruselling.org/index.php");//change
}
...
if(!empty($session)) {
	...
	if(!empty($user)){
		...
		//look for email
		$email_query = mysql_query("SELECT * FROM `rus_users` WHERE `ContactEmail` = '".$_SESSION['email']."'");
		...
		if($email_rows==1){
			...
			$add_user_query = mysql_query("INSERT INTO `campused_cbdbma1n`.`rus_users` (
				...
				VALUES ('', '".$_SESSION['username']."', 'fb', '".$_SESSION['id']."', '', '', '', '', '', '".$_SESSION['email']."', '', '', '', '', '', '')") or die();
	...
	$login_url = $facebook->getLoginUrl(array(
			'req_perms' => 'email,user_birthday,status_update,publish_stream,user_photos'
		));
